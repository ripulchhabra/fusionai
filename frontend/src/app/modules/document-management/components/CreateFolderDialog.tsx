...
const folderCreationSchema = Yup.object().shape({
  folderName: Yup.string()
    .min(3, 'Minimum 3 characters')
    .max(50, 'Maximum 50 characters')
    .required('Folder Name is required'),
  folderDescription: Yup.string().min(0, 'Minimum 3 characters').max(50, 'Maximum 50 characters'),
})

interface FolderCreateModel {
  folderName: string
...
const CreateFolderDialog = ({
  ...
  const initialValues: FolderCreateModel = {
    folderName: '',
    folderDescription: '',
  }
  ...
  const formik = useFormik({
    initialValues,
    validationSchema: folderCreationSchema,
    onSubmit: async (values) => {
      setLoading(true)
      createFolder(values.folderName, values.folderDescription, currentParent, currentCommunity)
        .then((response) => {
        ...
        .catch(() => {
          setFailureResMessage('Failed to create a folder')
          setChecked(true)
          setLoading(false)
        })
    },
